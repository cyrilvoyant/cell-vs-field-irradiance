...
if ~isempty(e) && isfield(e,'S')
    ...
    A(end+1) = struct('name', '\texttt{ERA5}', ...
                      ...
                      'theta', NaN, 'secs', NaN, 'gpr3', mean(S.gpr), ...
                      'fam', 'reference', 'nice3', mean(S.nicesig), 'gam3', mean(S.gamma));
end
end
...
if isfield(r,'met') && ~isempty(r.met)
    ...
    % NICE, THE MEAN GAMMA AND THE PASS RATE AS THE MEAN OF THE SIX REPORTED LEAD
    % TIMES, computed the same way for every row (Cyril, 14 September 2026).
    g  = mean_lead(r.met, 'gpr');
    n3 = mean_lead(r.met, 'nicesig');
    m3 = mean_lead(r.met, 'gamma');
end
...
fprintf(fid, [' & parameters & fit time (min) & \\NICE{}$^{\\Sigma}$' ...
              ' & mean $\\gamma$ & \\texttt{GPR} \\\\\n\\midrule\n']);
for k = 1:numel(S)
...
function v = mean_lead(met, f)
%MEAN_LEAD Mean of one metric over the reported lead times; NaN unless all exist.
v = NaN;
if isfield(met, f) && ~isempty(met.(f)) && all(isfinite(met.(f)))
    v = mean(met.(f));
end
end
